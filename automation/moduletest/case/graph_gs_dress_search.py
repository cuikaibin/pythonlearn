...
class GraphFunctionTests(Case):
    ...
    GraphFunctionTests
    """
    def test_gs_graph_dress_search(self): 
        """
        #需要执行的case
        """
        ...
        search_file=search_config.get_picture_name("dress")
        ...
        time.sleep(5)
        #assert (True == search_config.check_search_result(st,"dress"))
        log_check = search_config.check_search_result(st, "dress")
        if log_check == True:
            print "gs_graph_dress_search log check is ture"
        else:
            print "gs_graph_dress_search log check is false"
