...
class GraphFunctionTests(Case):
    ...
    GraphFunctionTests
    """
    def test_gs_graph_package_search(self): 
        """
        #需要执行的case
        """
        ...
        search_file=search_config.get_picture_name("package")
        ...
        time.sleep(5)
        #assert (True == search_config.check_search_result(st,"package"))
        log_check = search_config.check_search_result(st, "package")
        if log_check == True:
            print "gs_graph_package_search log check is ture"
        else:
            print "gs_graph_package_search log check is false"
