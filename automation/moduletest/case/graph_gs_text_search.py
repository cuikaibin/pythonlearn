...
class GraphFunctionTests(Case):
    ...
    GraphFunctionTests
    """
    def test_gs_graph_face_search(self): 
        """
        #需要执行的case
        """
        ...
        search_file=search_config.get_picture_name("text")
        ...
        time.sleep(5)
        #assert (True == search_config.check_search_result(st, "text"))
        log_check = search_config.check_search_result(st, "text")
        if log_check == True:
            print "gs_graph_face_search log check is ture"
        else:
            print "gs_graph_face_search log check is false"       
